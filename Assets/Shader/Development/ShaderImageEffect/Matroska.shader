Shader "Flecheria/ShaderImageEffect/Matroska"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			static float X2(float x) { return x * x; }
			...
			static float2 mouse;
			static float3 bg; // global background color
			...
			float4 Eyes(float2 uv, float seed) {
				...
				circ(eyes, uv, lp.x + pupil.x, lp.y - pupil.y, ar, size*.6, blur, 0.1); //pupil
				circ(eyes, uv, lp.x - glint.x, lp.y + glint.y, ar, size*.5, blur*.5, 1.0); //glint

				circ(eyes, uv, rp.x + pupil.x, rp.y - pupil.y, ar, size*.6, blur, 0.1); //pupil
				circ(eyes, uv, rp.x - glint.x, rp.y + glint.y, ar, size*.5, blur*.5, 1.0); //glint
			...
			float4 Face(float2 uv, float seed) {
				...
				circ(col, uv2, .52, .29, 2., .02, .005, 0.1);		// nostrils
			...
			float4 Flower(float2 uv, float4 pf, float4 baseCol, float4 lineCol) {
				...
				highlight *= SIN(st.x);
				col = lerp(col, 1.0, highlight);			// highlight

				col = lerp(col, 1.0, S(.06, .05, st.y));
			...
			de castRay(ray r) {
				...
				float3 p = 0.0;								// local position (after repeat, resize etc)
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 pos = mul(float3(0., camY, camDist), RotY(turn));
